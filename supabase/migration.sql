-- Beeliber Academy migration.sql
-- 목적: Supabase에서 Beeliber Academy의 핵심 스키마를 생성한다.
...
begin
  if not exists (select 1 from pg_type where typname = 'academy_user_role') then
    create type public.academy_user_role as enum (
      'student',
      'marketer',
      'developer',
      'automation',
  ...
  if not exists (select 1 from pg_type where typname = 'academy_user_status') then
    create type public.academy_user_status as enum (
      'active',
      'inactive',
      'suspended'
    );
  ...
  if not exists (select 1 from pg_type where typname = 'academy_module_difficulty') then
    create type public.academy_module_difficulty as enum (
      'basic',
      'intermediate',
      'advanced'
    );
  ...
  if not exists (select 1 from pg_type where typname = 'academy_submission_type') then
    create type public.academy_submission_type as enum (
      'text',
      'markdown',
      'json',
      'image',
  ...
  if not exists (select 1 from pg_type where typname = 'academy_submission_status') then
    create type public.academy_submission_status as enum (
      'draft',
      'submitted',
      'ai_reviewed',
      'revision_requested',
  ...
  if not exists (select 1 from pg_type where typname = 'academy_feedback_decision') then
    create type public.academy_feedback_decision as enum (
      'pass',
      'revise',
      'fail'
    );
  ...
  if not exists (select 1 from pg_type where typname = 'academy_resource_type') then
    create type public.academy_resource_type as enum (
      'markdown',
      'pdf',
      'link',
      'video',
  ...
  if not exists (select 1 from pg_type where typname = 'academy_rule_type') then
    create type public.academy_rule_type as enum (
      'brand',
      'policy',
      'language',
      'factual'
...
returns public.academy_user_role
language sql
stable
security definer
...
create table if not exists public.academy_module_resources (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint module_resource_payload_check
    check (
...
alter table public.academy_submission_files
  add constraint academy_submission_files_submission_id_fkey
  foreign key (submission_id) references public.academy_submissions(id) on delete cascade;
...
create trigger trg_academy_tracks_updated_at
...
create trigger trg_academy_module_resources_updated_at
before update on public.academy_module_resources
for each row execute function public.set_updated_at();

drop trigger if exists trg_academy_assignments_updated_at on public.academy_assignments;
create trigger trg_academy_assignments_updated_at
before update on public.academy_assignments
for each row execute function public.set_updated_at();

drop trigger if exists trg_academy_submissions_updated_at on public.academy_submissions;
create trigger trg_academy_submissions_updated_at
...
create trigger trg_academy_placement_updated_at
before update on public.academy_placement_recommendations
for each row execute function public.set_updated_at();

drop trigger if exists trg_academy_review_rules_updated_at on public.academy_review_rules;
create trigger trg_academy_review_rules_updated_at
before update on public.academy_review_rules
for each row execute function public.set_updated_at();

-- =========================
-- 6) Auth signup -> academy_users row
-- =========================
